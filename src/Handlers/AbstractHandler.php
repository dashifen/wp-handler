...
abstract class AbstractHandler implements HandlerInterface
{
    ...
    public static function findAutoloader (bool $throw = true): string
    {
        ...
        elseif ($autoloader = file_exists(ABSPATH . 'vendor/autoload.php'));
            ...
            // if we didn't find it in one of the locations above, then we'll
            // see if there's one adjacent to the current location.  this may
            // be the case during development, for example, but it's unlikely
            // in production environments.
            
            $autoloader = 'vendor/autoload.php';
            if ($throw && !file_exists(trailingslashit(__DIR__) . $autoloader)) {
                
                // if we're in here, then $throw was set and we couldn't find
                // an autoloader.  we know this because the three above tests
                // failed and now this most recent one did as well.  so, we'll
                // throw our exception.
                
                throw new HandlerException(
                    'Unable to find Composer autoloader',
                    HandlerException::NO_AUTOLOADER
                );
            }
        }
     ...
     * includeAutoloader
     *
     * This method includes composer's autoloader starting from Dash's
     * preferred location for it and then moving through progressively less
     * common locations until it tries to include one local to a plugin or
     * theme.
     *
     * @param bool $required
     *
    ...
    {
        // if the autoloader is required (and it usually is) then we assume
        // that we also want to throw an Exception that explains what's going
        // on when we don't find it.  this can always be overridden in the
        // concrete version of this class if someone wants, but it seems like
        // it's probably a fair guess in most cases.
        
        $autoloader = AbstractHandler::findAutoloader($required);
